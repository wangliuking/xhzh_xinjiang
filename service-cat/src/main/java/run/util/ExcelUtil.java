...
public class ExcelUtil {
    ...
    public static void exportExcel(HttpServletResponse response,
        ...
        String[] headers = { "行政区域", "机构名称", "站点名称", "RTU-ID","串口号","监测仪ID","型号","名称","安装位置","记录值","记录时间"};
        ...
        for (Map<String,Object> map : excelData) {
            ...
            row1.createCell(5).setCellValue(map.get("cathode_id")+"");
            row1.createCell(6).setCellValue(map.get("cathode_model")+"");
            row1.createCell(7).setCellValue(map.get("cathode_name")+"");
            row1.createCell(8).setCellValue(map.get("cathode_location")+"");
            row1.createCell(9).setCellValue(map.get("cathode_value")+"");
            row1.createCell(10).setCellValue(map.get("write_time")+"");
            rowNum++;
        }
